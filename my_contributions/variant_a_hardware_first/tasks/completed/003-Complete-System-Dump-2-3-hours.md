...
Instead of ADB dumping (which crashed), used existing `full_emmc_dump_clean_root.img` (7.3 GB) from `/backup/dumps/`

### Extraction Summary:

**GPT Partition Table (26 partitions):**
- Bootloader A/B (32 MB each)
- Boot A/B (64 MB each) 
- Vendor Boot A/B (32 MB each)
- Super A (2.0 GB - contains system+vendor logical partitions)
- DTBO A/B (2 MB each)
- Data partition (4.6 GB)

**Extracted Filesystems:**
- system_a.img (937 MB ext4) → mounted at partitions/system_mount
- vendor_a.img (110 MB ext4) → mounted at partitions/vendor_mount
- product_a.img (513 MB ext4)

**Boot Components:**
- bootloader_a.bin (32 MB)
- boot_a.bin (64 MB, Android bootimg format)
- vendor_boot_a.bin (32 MB, LZ4 compressed)
- dtbo_a.bin (2 MB, Device Tree Binary)
