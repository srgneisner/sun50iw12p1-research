...
#define USB_TIMEOUT	120000 /* 120 seconds for H713 large transfers */
...
{
	int rc, recv;
	unsigned char temp_buffer[65];  /* Extra byte for safety */
	unsigned char *recv_ptr = (unsigned char *)data;
	int buffer_size = 65;  /* Always use larger buffer for H713 compatibility */
	
	while (length > 0) {
		/* For small reads, use temp buffer to avoid H713's 64-byte responses */
		if (length <= 64) {
			int copy_len;
			rc = libusb_bulk_transfer(usb, ep, temp_buffer, 65,
						  &recv, USB_TIMEOUT);
			if (rc != 0)
				usb_error(rc, "usb_bulk_recv()", 2);
			/* Copy only the requested amount */
			copy_len = (recv < length) ? recv : length;
			memcpy(recv_ptr, temp_buffer, copy_len);
			length -= copy_len;
			recv_ptr += copy_len;
		} else {
			/* For large reads, receive directly */
			rc = libusb_bulk_transfer(usb, ep, recv_ptr, length,
						  &recv, USB_TIMEOUT);
			if (rc != 0)
				usb_error(rc, "usb_bulk_recv()", 2);
			length -= recv;
