...
/* H713 uses SRAM A2 with different swap buffer addresses */
...
soc_info_t soc_info_table[] = {
	{
		...
		.name         = "H713",
		.spl_addr     = 0x104000,  /* SRAM A2 base, from boot0.bin header */
		.scratch_addr = 0x121000,  /* FEL scratch after SPL in SRAM A2 */
		.thunk_addr   = 0x123a00, .thunk_size = 0x200,  /* Thunk in SRAM A2 */
		.swap_buffers = h713_sram_swap_buffers,  /* H713-specific SRAM A2 layout */
		.sram_size    = 128 * 1024,  /* 128 KB SRAM A2 */
		.sid_base     = 0x03006000,
		.sid_offset   = 0x200,
		.sid_sections = generic_2k_sid_maps,
		.rvbar_reg    = 0x09010040,
