...
#define USB_TIMEOUT	20000 /* 20 seconds (H713 needs longer for SRAM A2 transfers) */
 ...
 *
 * H713: Reduced from 512 KiB to 16 KiB to avoid USB timeout issues
 * with BROM's slow SRAM A2 access patterns
 */
static const int AW_USB_MAX_BULK_SEND = 16 * 1024; /* 16 KiB per bulk request (H713 limit) */
...
{
	int rc, recv;
	unsigned char temp_buffer[65];  /* Extra byte for safety - H713 returns 64 bytes */
	unsigned char *recv_ptr;
	int buffer_size;
	
	/* H713 workaround: For small reads, use larger buffer */
	/* H713 BROM sends 64-byte responses instead of the requested size */
	if (length <= 64) {
		recv_ptr = temp_buffer;
		buffer_size = 65;
	} else {
		recv_ptr = (unsigned char *)data;
		buffer_size = length;
	}
